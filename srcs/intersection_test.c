...
static bool	intersection_test_sphere(const t_sphere *shpere, \
...
static bool	intersection_test_plane(const t_plane *plane, \
				const t_ray *ray, t_intersection_point *out_intp)
{
	double	dn_dot;
	double	t;
	...
	start_origin = sub(ray->start, plane->pos);
	dn_dot = dot(&ray->direction, &normal);
	if (dn_dot != 0)
	{
		t = -dot(&start_origin, &normal) / dn_dot;
		if (t > 0)
		{
			out_intp->distance = t * norm(&ray->direction);
			out_intp->pos = get_position(t, ray);
...
static bool	intersection_test_cylinder(const t_cylinder *cylinder, \
				const t_ray *ray, t_intersection_point *out_intp)
{
	t_ray	cyl_ray;
	double	form[FORMULA_NUM];
	...
	cyl_ray.start = cross(cylinder->norm_ori_vec, \
						sub(ray->start, cylinder->pos));
	...
	out_intp->normal = sub(out_intp->pos, \
						add(cylinder->pos, \
